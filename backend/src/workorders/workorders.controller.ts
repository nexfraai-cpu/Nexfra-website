...
export class WorkordersController {
  ...
  getById = asyncWrap(async (req: Request, res: Response) => {
    const wo = await this.workordersService.getById(req.params.id, req.user!);
    res.status(200).json({ data: wo });
  });

  create = asyncWrap(async (req: Request, res: Response) => {
    const wo = await this.workordersService.create(req.body, req.user!);
    res.status(201).json({ data: wo });
  });

  update = asyncWrap(async (req: Request, res: Response) => {
    const wo = await this.workordersService.update(req.params.id, req.body, req.user!);
    res.status(200).json({ data: wo });
  });

  delete = asyncWrap(async (req: Request, res: Response) => {
    await this.workordersService.softDelete(req.params.id, req.user!);
    res.status(200).json({ data: { message: 'Work order deleted successfully' } });
  });

  setDueDate = asyncWrap(async (req: Request, res: Response) => {
    const wo = await this.workordersService.setDueDate(req.params.id, req.body.dueDate, req.user!);
    res.status(200).json({ data: wo });
  });

  toggleUrgent = asyncWrap(async (req: Request, res: Response) => {
    const wo = await this.workordersService.toggleUrgent(req.params.id, req.user!);
    res.status(200).json({ data: wo });
  });
}
