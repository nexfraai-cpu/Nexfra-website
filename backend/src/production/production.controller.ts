...
export class ProductionController {
  ...
  getById = asyncWrap(async (req: Request, res: Response) => {
    const item = await this.productionService.getById(req.params.id, req.user!.id);
    res.status(200).json({ data: item });
  });

  update = asyncWrap(async (req: Request, res: Response) => {
    const item = await this.productionService.update(req.params.id, req.body, req.user!.id);
    res.status(200).json({ data: item });
  });

  advanceStage = asyncWrap(async (req: Request, res: Response) => {
    const item = await this.productionService.advanceStage(req.params.id, req.body, req.user!.id);
    res.status(200).json({ data: item });
  });

  getChassisRecords = asyncWrap(async (req: Request, res: Response) => {
    const records = await this.productionService.getChassisRecords(req.params.id, req.user!.id);
    res.status(200).json({ data: records });
  });

  addChassis = asyncWrap(async (req: Request, res: Response) => {
    const record = await this.productionService.addChassis(req.params.id, req.body, req.user!.id);
    res.status(201).json({ data: record });
  });

  updateChassis = asyncWrap(async (req: Request, res: Response) => {
    const record = await this.productionService.updateChassis(req.params.id, req.params.chassisId, req.body, req.user!.id);
